...
impl fmt::Display for Command {
 ...
 impl Command{
    ...
    pub fn new(name : String, args: Vec<String>) -> Command {
        Command{ command_name: name,command_args: args}
    }

    pub fn run_command (&self){
        let status = std::process::Command::new(self.command_name.clone()).args(&self.command_args).status().expect("failed to execute command");         
        if !status.success(){
